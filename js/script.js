...
pokeSearch.addEventListener('keyup',(event)=>{
    ...
    fetch(`https://pokeapi.co/api/v2/pokemon?limit=1500`).then(response => response.json()).then(result=>{
    ...
    const listHtml = pokemonList.map(pokemon => `<li>${pokemon.name.charAt(0).toUpperCase()+pokemon.name.substring(1).toLowerCase()}</li>`).join("")
